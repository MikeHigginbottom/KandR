...
char *fold(char *input, char *output, int requestedFoldPoint) {
	...
	char *unfoldedSubstrStart = input;
	char *foundFoldPos = NULL;
	while (*inputPos != '\0') {
		if ((inputPos - unfoldedSubstrStart) + 1 >= requestedFoldPoint) {
			if (*inputPos == ' ' || *inputPos == '\t' || *inputPos == '\n') {
				foundFoldPos = inputPos;
			}
			if (foundFoldPos != NULL) {
				while (unfoldedSubstrStart < foundFoldPos) {
					*outputPos++ = *unfoldedSubstrStart++;
				}
				*outputPos++ = '.';
				foundFoldPos = NULL;
				while (*unfoldedSubstrStart == ' ' || *unfoldedSubstrStart == '\t' || *unfoldedSubstrStart == '\n') {
					unfoldedSubstrStart++;
				}
				inputPos = unfoldedSubstrStart;
			}
			else {
				while (unfoldedSubstrStart < inputPos) {
					*outputPos++ = *unfoldedSubstrStart++;
				}
				*outputPos++ = '.';
				inputPos = unfoldedSubstrStart;
